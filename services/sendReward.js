        ...
        if (stakeRewardList.length > 0) {
            ...
            for (var i = 0; i < stakeRewardList.length; i++) {
                try {
                    ...
                    await AutomateStakeModel.updateOne({ _id: stakeRewardList[i]._id }, {
                        $set: {
                            isRewardTransfer: true
                        }
                    })
                    let encoded_tx = await mint(stakeRewardList[i].wallet, parseInt(stakeRewardList[i].rewardAmount) * 1e2)
