...
// const runTask = () => {
//     // Your task to be executed every 24 hours
//     console.log('Task executed every 24 hours');
// };
...
//     setTimeout(() => {
//       runService(); // Run the initial task at midnight
//       setInterval(runService, interval); // Run the task every 24 hours after the initial run
//     }, initialDelay);
//   };
...
const runTask = async () => {
    ...
    try {
        ...
        try {
            let { data } = await axios.get("https://www.bitmex.com/api/v1/funding?symbol=ETHUSD&count=1&reverse=true")
        ...
        if (walletList.length > 0) {
            ...
            for (var i = 0; i < walletList.length; i++) {
                ...
                if (walletList[i].balance >= parseFloat(balance / 1e2)) {
                    ...
                    const stake = new AutomateStakeModel({ wallet: walletList[i].account, stakeAmount: walletList[i].balance, rewardPercentage, percentage, rewardAmount: (walletList[i].balance * percentage) / 100 });
                    await stake.save()
                    await WalletModel.updateOne({ _id: walletList[i]._id }, {
                        $set: {
